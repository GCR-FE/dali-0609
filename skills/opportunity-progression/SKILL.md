...
These guarantee the externalized detail is in context at the moment it is used. Reading is per-need, not all-upfront.
...
4. **Never infer, summarize, or reconstruct** a Model rule or View template from memory. If the node is not in current context, read the file. The tested output path depends on the verbatim rules.
