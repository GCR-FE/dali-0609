...
- 分析结论可在 300 字内讲清 → 直接 inline
- Skill 产出内容 > 800 字 → 摘要（≤200中文字）+ PDF 文件卡片
- 结构化产出（报告/画布/评分卡/对比矩阵）→ 永远摘要 + PDF 文件卡片，无论长度
...
**执行规则：**
- 每个 skill 产出遵循 Control → Record(.md) → View(HTML→PDF) 三步
...
- Skill 产出: 聊天只发摘要（≤200字），完整内容自动生成 HTML→PDF 文件交付
...
- If user name is known → greet by name, then offer help
- If unknown → ask once ("怎么称呼你？"), save to USER.md, then never ask again in future sessions
